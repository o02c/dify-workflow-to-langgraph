...
- **ノード本体**は全タイプが Node Handler レジストリ（ADR-0005、実装済み `codegen/handlers.py`）経由で型付き出力を生成。`start` / `end` は宣言された変数から出力フィールドを導出。`end`（value_selector 転送、ADR-0004）と `knowledge-retrieval`（`Retriever` ポート呼び出し、ADR-0006）は決定論的な実本体を生成。その他は Stub、未登録タイプは汎用 Stub にフォールバック。Stub 本体の LLM 埋めは任意後処理（ADR-0001）。分岐 Stub は決定フィールドを先頭ブランチにデフォルトするため、本体未実装でもグラフは単一経路で走る。
...
- **Codegen**: ノードタイプごとの Node Handler（`codegen/handlers.py`）が `output_fields` / `stub_output` / `body_imports` と分岐情報（`is_branching` / `decision_field`）を担う（ADR-0005）。構造的な分岐マップ導出は `codegen/routing.py`。生成物は自己完結パッケージ（相対 import・1 ノード 1 ファイル、ADR-0007、実装済み）。
...
- 現状出力: `__init__.py` / `__main__.py` / `state.py` / `graph.py` / `nodes/<node>.py` / `llm.py` / `retriever.py`（相対 import、`python -m <pkg>` で実行）
- 目標追加（未実装）: `env.py`（ADR-0004）
